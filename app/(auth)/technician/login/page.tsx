...
import { Loader2, Clock, ShieldCheck, Home, Lock, Phone, ArrowRight, CheckCircle2, Wrench } from "lucide-react"
...
function TechnicianLoginContent() {
  ...
  useEffect(() => {
    document.title = "MaintainU Field"
  }, [])
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!status.exists) {
        ...
        if (status.role && status.role !== "technician") {
          toast.error(`This account is registered as ${status.role}. Please log in via the ${status.role} portal.`)
          return
        }
        if (status.status === 'PENDING_APPROVAL' || status.status === 'PENDING_PROFILE') {
          setStep("waiting")
        } else if (status.status === 'ACTIVE') {
          const res = await api.sendOTP(phone, "technician")
  ...
  const handleVerifyOTP = async (codeStr: string) => {
    ...
    try {
      ...
      if (res.success) {
        toast.success("Welcome back to MaintainU Field!")
        router.push('/technician/dashboard')
      } else {
  ...
  return (
    <div className="app-technician min-h-screen flex items-center justify-center p-4 sm:p-6 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-slate-950 dark:to-slate-900">
      <div className="w-full max-w-md glass p-6 sm:p-8 rounded-2xl sm:rounded-3xl shadow-xl">
        ...
        <div className="mb-6 sm:mb-8 space-y-2 sm:space-y-3 text-center">
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">
            {step === "login" && "MaintainU Field"}
            {step === "verify-required" && "Technician Verification Required"}
            {step === "waiting" && "Verification Pending"}
            {step === "approved" && "Verified!"}
            {step === "otp-entry" && "Enter Verification Code"}
          </h1>
          <p className="text-sm sm:text-base text-muted-foreground">
            {step === "login" && "Sign in to accept and manage field jobs"}
            {step === "verify-required" && "Your technician account requires admin verification"}
        ...
        {isLoading ? (
          ...
          <div className="space-y-6 text-center animate-in fade-in duration-300">
            ...
            <div className="space-y-2 sm:space-y-3">
              ...
              <p className="text-muted-foreground text-xs sm:text-sm leading-relaxed px-2">
                Your technician account is currently under review. Please wait here while we verify your details.
              </p>
            </div>
            ...
            <div className="bg-white/50 dark:bg-card/50 rounded-2xl p-4 text-xs sm:text-sm text-left border border-border/50">
              ...
              <p className="text-muted-foreground pl-7 sm:pl-8">
                An admin will verify your field technician details. Once approved, you will be automatically redirected.
              </p>
            </div>
            ...
            >
              Re-submit Technician Verification Request
            </button>
          </div>
          ...
          <div className="space-y-6 text-center animate-in fade-in duration-300">
            ...
            <div className="space-y-2 sm:space-y-3">
              <h2 className="text-lg sm:text-xl font-bold tracking-tight text-foreground">Technician Verification Required</h2>
              <p className="text-muted-foreground text-xs sm:text-sm leading-relaxed px-2">
                To ensure platform security, all technician accounts require administrator verification before access is granted.
              </p>
            </div>
            ...
            <div className="bg-white/50 dark:bg-card/50 rounded-2xl p-4 text-xs sm:text-sm text-left border border-border/50">
              ...
              <p className="text-muted-foreground text-[10px] sm:text-xs">
                By clicking "Submit for Verification", your phone number will be sent to our administrators for technician approval.
              </p>
            </div>
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            >
              {isLoading && <Loader2 className="w-4 h-4 animate-spin" />}
              {isLoading ? "Sending OTP..." : "Sign In to Field"}
            </button>
          </div>
        )}
        ...
        {step === "login" && (
          <div className="mt-8 pt-6 border-t border-border/50 text-center space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              ...
              <button
                onClick={() => router.push("/signup?role=technician")}
